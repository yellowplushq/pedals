...
final class TerminalSurfaceCoordinator {
    ...
    private var lastSentPixelWidth: UInt32 = 0
    private var lastSentPixelHeight: UInt32 = 0
    ...
    func synchronizeMetrics() {
        ...
        // Ghostty applies `setSize` to terminal state asynchronously on its
        // termio thread, while host-managed writes parse synchronously on
        // the caller. Re-arming in-band size reports *before* a new size is
        // queued guarantees the applied resize emits a report, which is the
        // only ordering-safe trigger for propagating the new grid to a
        // remote pty. (Re-arming also self-heals after a terminal reset,
        // which clears the mode.)
        if pixelWidth != lastSentPixelWidth || pixelHeight != lastSentPixelHeight {
            lastSentPixelWidth = pixelWidth
            lastSentPixelHeight = pixelHeight
            configuration.inMemorySession?.armInBandSizeReports()
        }
        ...
        // Delegates receive the projected grid synchronously for view-side
        // geometry (selection overlays, IME). The in-memory session's resize
        // callback deliberately does NOT fire here: ghostty has not applied
        // this grid to terminal state yet, and announcing it to a remote pty
        // now would let the pty's redraw race the local grid resize.
