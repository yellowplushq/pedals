...
public final class InMemoryTerminalSession: @unchecked Sendable {
    ...
    /// - Parameters:
    ///   - write: Receives bytes the terminal wants delivered to the host
    ///     transport (keystrokes, query replies). In-band size reports are
    ///     stripped out before this fires.
    ///   - resize: Fires after Ghostty has *applied* a grid resize to
    ///     terminal state (signalled by a mode 2048 in-band size report).
    ///     Forwarding this viewport to a remote pty is therefore safe: any
    ///     output produced for it is parsed against a grid of at least this
    ///     size.
    public init(
    ...
    func setSurface(_ surface: ghostty_surface_t?) {
        ...
        if surface != nil {
            armInBandSizeReports()
        }
    }

    /// Enable mode 2048 in-band size reports on the attached surface.
    ///
    /// The terminal then emits a size report from the same termio critical
    /// section that applies each grid resize — the ordering barrier this
    /// session's `resize` callback is built on — plus one immediate report
    /// for the current grid. Idempotent; call again after feeding a reset
    /// (RIS) since a full reset clears the mode.
    public func armInBandSizeReports() {
        guard surfaceAccess.writeSynchronously(
            InMemoryTerminalSizeReport.enableSequence
        ) else {
            TerminalDebugLog.log(.metrics, "size report arm skipped: missing surface")
            return
        }
        TerminalDebugLog.log(.metrics, "size reports armed")
    }
    ...
    /// Process host data on the caller's executor before returning.
    ///
    /// A UI host may use this from the same main-actor path that renders the
    /// surface, preventing a renderer tick from racing terminal parsing.
    public func receiveSynchronously(_ data: Data) {
        guard surfaceAccess.writeSynchronously(data) else {
            TerminalDebugLog.log(
                .output,
                "terminal <- host dropped \(TerminalDebugLog.describe(data))"
            )
        ...
        TerminalDebugLog.log(
            .output,
            "terminal <- host synchronous \(TerminalDebugLog.describe(data))"
        )
    }
    ...
    static let receiveBufferCallback: ghostty_surface_receive_buffer_cb = { userdata, ptr, len in
        ...
        session.receiveFromTerminal(data)
    }

    /// Runs on ghostty's termio thread. A size report emitted here has the
    /// grid resize it describes already applied to terminal state, so it is
    /// dispatched as an applied-resize event; everything else is host input.
    private func receiveFromTerminal(_ data: Data) {
        let (passthrough, reports) = InMemoryTerminalSizeReport.extract(from: data)

        for report in reports {
            TerminalDebugLog.log(
                .metrics,
                "size report applied cols=\(report.columns) rows=\(report.rows) pixels=\(report.widthPixels)x\(report.heightPixels)"
            )
            dispatchResize(report.viewport)
        }

        guard !passthrough.isEmpty else { return }
        TerminalDebugLog.log(
            .input,
            "host <- terminal \(TerminalDebugLog.describe(passthrough))"
        )
        writeHandler(passthrough)
    }
